...
### CPU의 구조와 구성 요소
- 구성요소:
   - 1. 연산 유닛 (ALU: Arithmetic Logic Unit):
     - 산술 연산과 논리 연산 등을 수행. 
     - 입력된 데이터에 대해 연산을 수행하고, 그 결과를 레지스터로 전달하거나 메모리에 저장. 
   - 2. 제어 유닛 (CU: Control Unit):
...
2. Decode
   ...
   - 동작:
     - CU가 명령어를 분석.
     - 명령어가 산술 연산, 논리 연산, 데이터 이동 등 무엇을 해야 하는지 해석된다.
...
- 정의: 데이터를 하나씩 순차적으로 접근.
- 특징:
  - 데이터가 순차적으로 저장.
  - 원하는 데이터에 접근하기 위해 앞의 데이터를 반드시 지나쳐야 함.
- 장점: 구현이 간단하고 저렴.
- 단점: 데이터 접근 시간이 길어짐.
#### 2. 직접 접근 (Direct Access)
- 정의: 데이터가 메모리 내에 저장된 위치에 관계없이 직접 접근 가능. 
- 특징:
  - 원하는 위치에 바로 접근 가능.
  - 특정 위치에 데이터를 빠르게 읽거나 쓸 수 있음.
- 장점: 빠른 속도 제공.
- 단점: 데이터 전송 속도는 장치의 물리적 한계에 따라 달라질 수 있음. 
...
- 역할 
  - 1. 속도 차이 해결: CPU와 주 메모리 간의 속도 차이를 줄여준다.
  - 2. 자주 사용되는 데이터 저장: CPU가 자주 사용하는 데이터를 저장하며, 해당 데이터를 반복적으로 빠르게 접근할 수 있게 한다.
...
- 캐시 미스 (Cache Miss)
  ...
  - CPU는 주 메모리에서 데이터를 가져와야 하므로 시간이 더 소요된다. 
...
- 동작 방식
  ...
  4. 가상 주소는 TLB나 페이지 테이블을 통해 물리 주소로 변환됨. 물리 주소를 통해 데이터를 가져옴. 
- 장점: 메모리 접근 시간 단축. 
